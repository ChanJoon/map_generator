...
Maps::pcl2ros()
{
  ...
  RCLCPP_INFO(info.node->get_logger(),
              "finish: infill %lf%%",
              info.cloud->width /
                (1.0 * info.sizeX * info.sizeY * info.sizeZ));
}
...
Maps::perlin3D()
{
  ...
  RCLCPP_INFO(info.node->get_logger(),
              "the number of points before optimization is %d",
              info.cloud->width);
  info.cloud->points.resize(info.cloud->width * info.cloud->height);
  pcl2ros();
}
...
{
  RCLCPP_INFO(info.node->get_logger(),
              "generating maze with width %d , height %d",
              xh - xl + 1,
              yh - yl + 1);
...
Maps::Maze3DGen()
{
  ...
  for (int i = 0; i < numNodes; i++)
  {
    ...
    RCLCPP_INFO(
      info.node->get_logger(), "point: x: %f , y: %f , z: %f", rx, ry, rz);
