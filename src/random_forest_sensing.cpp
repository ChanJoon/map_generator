...
{
public:
  MapGenerator() : Node("map_generator_node")
  {
    ...
    _odom_sub = this->create_subscription<nav_msgs::msg::Odometry>("odometry", 50, std::bind(&MapGenerator::rcvOdometryCallbck, this, std::placeholders::_1));

    timer_ = this->create_wall_timer(
        std::chrono::milliseconds(static_cast<int>(1000 / _sense_rate)),
        std::bind(&MapGenerator::pubSensedPoints, this));
  }
...
private:
  ...
  void pubSensedPoints()
  {
    ...
    {
      RCLCPP_WARN(this->get_logger(), "[Map server] No obstacles .");
      return;
    }
